...
- [Python Katas](#python-katas)
- [Mega Project List](#mega-project-list)
...
[**Find PI to the Nth Digit**](./numbers/pi^n.py) :heavy_check_mark: Enter a number and have the program generate PI up to that many decimal places. Keep a limit to how far the program will go.

[**Find e to the Nth Digit**](./numbers/e^n.py) :heavy_check_mark: Just like the previous problem, but with e instead of PI. Enter a number and have the program generate e up to that many decimal places. Keep a limit to how far the program will go.

[**Fibonacci Sequence**](./numbers/fibonacci.py) :heavy_check_mark: Enter a number and have the program generate the Fibonacci sequence to that number or to the Nth number.

**Prime Factorization** - Have the user enter a number and find all Prime Factors (if there are any) and display them.

**Next Prime Number** - Have the program find prime numbers until the user chooses to stop asking for the next one.

[**Find Cost of Tile to Cover W x H Floor**](./numbers/tilecost.py) :heavy_check_mark: Calculate the total cost of tile it would take to cover a floor plan of width and height, using a cost entered by the user.

**Mortgage Calculator** - Calculate the monthly payments of a fixed term mortgage over given Nth terms at a given interest rate. Also figure out how long it will take the user to pay back the loan. For added complexity, add an option for users to select the compounding interval (Monthly, Weekly, Daily, Continually).

[**Change Return Program**](./numbers/changereturn.py) :heavy_check_mark: The user enters a cost and then the amount of money given. The program will figure out the change and the number of quarters, dimes, nickels, pennies needed for the change.

**Binary to Decimal and Back Converter** - Develop a converter to convert a decimal number to binary or a binary number to its decimal equivalent.

**Calculator** - A simple calculator to do basic operators. Make it a scientific calculator for added complexity.

[**Unit Converter (temp, currency, volume, mass and more)**](./numbers/unit_converter.py) :construction_worker: Converts various units between one another. The user enters the type of unit being entered, the type of unit they want to convert to and then the value. The program will then make the conversion.
...
[**Number Names**](./numbers/number_names.py) :construction_worker: Show how to spell out a number in English. You can use a preexisting implementation or roll your own, but you should support inputs up to at least one million (or the maximum value of your language's default bounded integer type, if that's less). *Optional: Support for inputs other than positive integers (like zero, negative integers, and floating-point numbers).*

[**Coin Flip Simulation**](./numbers/coinflip.py) :heavy_check_mark: Write some code that simulates flipping a single coin however many times the user decides. The code should record the outcomes and count the number of tails and heads.
...
[**Collatz Conjecture**](./algorithms/collatz.py) :heavy_check_mark: Start with a number *n > 1*. Find the number of steps it takes to reach one using the following process: If *n* is even, divide it by 2. If *n* is odd, multiply it by 3 and add 1.
...
[**Fizz Buzz**](text/fizzbuzz.py) :heavy_check_mark: Write a program that prints the numbers from 1 to 100. But for multiples of three print “Fizz” instead of the number and for the multiples of five print “Buzz”. For numbers which are multiples of both three and five print “FizzBuzz”.

[**Reverse a String**](text/reversestring.py) :construction_worker: Enter a string and the program will reverse it and print it out.

[**Pig Latin**](./text/piglatin.py) :heavy_check_mark: Pig Latin is a game of alterations played on the English language game. To create the Pig Latin form of an English word the initial consonant sound is transposed to the end of the word and an ay is affixed (Ex.: "banana" would yield anana-bay). Read Wikipedia for more information on rules.

[**Count Vowels**](./text/vowelcount.py) :heavy_check_mark: Enter a string and the program counts the number of vowels in the text. For added complexity have it report a sum of each vowel found.

[**Check if Palindrome**](./text/palindrome.py) :construction_worker: Checks if the string entered by the user is a palindrome. That is that it reads the same forwards as backwards like “racecar”

[**Count Words in a String**](./text/wordcount.py) :construction_worker: Counts the number of individual words in a string. For added complexity read these strings in from a text file and generate a summary.

**Text Editor** - Notepad style application that can open, edit, and save text documents. *Optional: Add syntax highlighting and other features.*

[**RSS Feed Creator**](./text/rss_feed.py) :construction_worker: Given a link to RSS/Atom Feed, get all posts and display them.
...
[**Vigenere / Vernam / Ceasar Ciphers**](./text/ciphers.py) :construction_worker: Functions for encrypting and decrypting data messages. Then send them to a friend.
...
[**Product Inventory Project**](./classes/inventory.py) :construction_worker: Create an application which manages an inventory of products. Create a product class which has a price, id, and quantity on hand. Then create an *inventory* class which keeps track of various products and can sum up the inventory value.
